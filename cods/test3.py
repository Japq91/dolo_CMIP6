#!/usr/bin/env python3
# p03_thredds_ncss.py  (mensual)
import argparse, os, re, shlex, subprocess, calendar
...
def parse_fname_year_version(fname: str):
    """
    Devuelve (year:int|None, ver:str or '') para nombres tipo ..._2015_v2.0.nc o ..._2015.nc
    """
...
def make_monthly_fname(fname: str, year: int, month: int):
    """
    Inserta YYYYMM antes de la versión si existe.
    ..._2015_v2.0.nc -> ..._201501_v2.0.nc
    ..._2015.nc      -> ..._201501.nc
    """
...
def monthly_bounds(year: int, month: int, hour: int = 12):
...
def main():
    ap = argparse.ArgumentParser(description="Descarga mensual vía NCSS; guarda en ../data/<MODELO>/")
    ...
    for url in lines:
        # Base NCSS y nombre anual original
        base, var, fname = build_ncss_base_and_fname(url, var=args.var)
        year, ver = parse_fname_year_version(fname)
        if year is None:
        ...
        # 12 meses
        for m in range(1, 13):
            t0, t1 = monthly_bounds(year, m, args.hour)
            # Construye query
            q = [f"var={var}"]
            if args.bbox:
